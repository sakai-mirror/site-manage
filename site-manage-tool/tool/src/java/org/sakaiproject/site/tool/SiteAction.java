 ...
 * $Id: SiteAction.java 20107 2007-01-04 19:52:49Z zqian@umich.edu $
 ***********************************************************************************
 *
...
public class SiteAction extends PagedResourceActionII {
	...
	private String ALL_MY_SITES = rb.getString("java.allmy");// "All My
	...
	/**
	 * The alert message shown when no site has been selected for the requested
	 * action.
	 */
	private String NO_SITE_SELECTED_STRING = rb.getString("java.nosites");

	/**
	 * The alert message shown when Revise... has been clicked but more than one
	 * site was checked
	 */
	private String MORE_THAN_ONE_SITE_SELECTED_STRING = rb
			.getString("java.please");
			...
			try {
				...
				if (allowUpdateSite) {
					...
					if (!isMyWorkspace
						...
						// show the group toolbar unless configured to not
						// support group
					...
					if (!isMyWorkspace) {
						...
						if (siteType == null || siteType != null
								&& !isGradToolSite) {
							// hide site duplicate and import for GRADTOOLS type
							// of sites
							...
							if (updatableSites.size() > 0) {
								...
								// a configuration param for showing/hiding
								// import from file choice
								...
								if (importFromFile.equalsIgnoreCase("true")) {
									// htripath: June 4th added as per Kris and
									// changed desc of above
					...
					// if the page order helper is available, not stealthed and
					// not hidden, show the link
					...
					if (lst != null && lst.size() > 0) {
						...
						while (iter.hasNext()) {
							ImportMetadata importdata = (ImportMetadata) iter
									.next();
							// Log.info("chef","Preparing import item '" +
							// importdata.getId() + "'");
			...
			if (SecurityService.isSuperUser()) {
				...
				if (view != null) {
					if (view.equals(ALL_MY_SITES)) {
						...
						// search for a specific user site for the particular
						// user id in the criteria - exact match only
						try {
				...
				if (manualAddNumber != 0) {
					...
					for (int j = 0; j < manualAddNumber; j++) {
						manualSections = manualSections.concat(
								CourseIdGenerator.getCourseId(term,
			...
			try {
				...
				if (AuthzGroupService.allowUpdate(realmId)) {
					try {
						...
						realmEdit.addMember(user.getId(), role.getId(), true,
								false);
						AuthzGroupService.save(realmEdit);
					} catch (Exception ignore) {
					}
	...
	public void doUpdate_participant(RunData data) {
		...
		if (AuthzGroupService.allowUpdate(realmId)
			...
			try {
				...
				if (hadMaintainUser != realmEdit.getUsersHasRole(
						maintainRoleString).isEmpty()) {
		...
		case 28:
			...
			if (forward) {
				if (params.getStrings("importSites") == null) {
					...
					Hashtable sites = (Hashtable) state
							.getAttribute(STATE_IMPORT_SITES);
					if (sites == null) {
						sites = new Hashtable();
					}
					...
					if (StringUtil.trimToNull(params.getString("title")) == null) {
						...
						try {
							...
							try {
								...
								// if goes here, IdService or SiteService has
							...
							// TODO: hard coding this frame id is fragile,
							// portal dependent, and needs to be fixed -ggolden
							// schedulePeerFrameRefresh("sitenav");
							scheduleTopRefresh();
